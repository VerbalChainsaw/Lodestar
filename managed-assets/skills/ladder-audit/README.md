...
7. runs only the bounded transfer probes needed to reduce uncertainty;
...
```text
Use Ladder Audit on this outcome. Preserve the unchanged graduation request, freeze proven capabilities, start at the deepest unproven boundary, remove one capability-class scaffold per rung, qualify reset and external oracles, identify evidence-backed pre-fixes, and recommend the smallest evidence-useful next tranche and its starting rung.
```
