...
function commandTokens(command) {
  ...
  const shellWrapped = value.match(
    /^(?:\S*[\\/])?(?:ba|da|z)?sh(?:\.exe)?\s+-lc\s+(['"])([\s\S]*)\1$/i,
  );
  const powershellWrapped = value.match(
    /^(?:\S*[\\/])?(?:powershell|pwsh)(?:\.exe)?(?:\s+-NoProfile)?\s+-(?:Command|c)\s+(['"])([\s\S]*)\1$/i,
  );
  if (shellWrapped) value = shellWrapped[2];
  else if (powershellWrapped) value = powershellWrapped[2];
...
function lodestarInvocation(event) {
  if (event?.type !== "tool") return null;
  const tokens = commandTokens(event.command);
  const index = tokens.findIndex((token) =>
    /^agentctx(?:\.mjs|\.cmd|\.exe|\.ps1)?$/i.test(commandBasename(token)));
  if (index < 0) return null;
  const operation = String(tokens[index + 1] ?? "").toLowerCase();
  if (!["start", "get", "resolve", "find"].includes(operation)) return null;
  const directlyInvoked = index === 0
    || (
      index === 1
      && (
        /^node(?:\.exe)?$/i.test(commandBasename(tokens[0]))
        || tokens[0] === "&"
      )
    )
    || (
      index === 2
      && tokens[0] === "&"
      && /^node(?:\.exe)?$/i.test(commandBasename(tokens[1]))
    );
  if (!directlyInvoked && !["&&", ";"].includes(tokens[index - 1])) return null;
  return { operation, directlyInvoked };
}
...
export function scoreCategoryTrial({
  ...
  const usedLodestar = toolEvents.some((event) =>
    lodestarInvocation(event) !== null);
  const requiredLodestar = condition === "lodestar";
  const firstInvocation = lodestarInvocation(toolEvents[0]);
  const protocolCompliant = !requiredLodestar
    || (
      firstInvocation?.operation === "start"
      && firstInvocation.directlyInvoked
    );
