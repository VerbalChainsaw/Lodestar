...
import { buildIndexes, searchTerms } from "./indexes.mjs";
import {
  comparablePath,
  resolveProjectAt,
...
export class ContextStore {
  static async open({
    ...
    if (selected) {
      resolved = {
        cwd: await (fsApi.realpath ?? fs.realpath)(cwd),
        project: selected,
        root: selected.roots?.[0] ?? null,
      };
    } else {
  ...
  async put(record) {
    validateRecord(record);
    await withWriteLock({ home: this.home }, async () => {
      const fresh = await ContextStore.open({
        home: this.home,
        cwd: this.cwd,
        project: this.project?.id ?? null,
      });
      const source = await fresh.sourceSnapshot();
      let records;
      if (record.scope.includes("global") && record.id.startsWith("g:")) {
        records = source.globalRecords;
      } else {
        const projectScope = record.scope.find((scope) =>
          scope.startsWith("project:"));
        const projectId = projectScope?.slice("project:".length);
        if (
          !projectId
          || !record.id.startsWith(`${projectId}:`)
          || !source.projectRecords[projectId]
        ) {
          throw new ContextError("record-scope-not-found", { id: record.id });
        }
        records = source.projectRecords[projectId];
      }
      const existing = records.findIndex(({ id }) => id === record.id);
      ...
      await promoteGeneration({ home: this.home, generation });
      await (this.fsApi.appendFile ?? fs.appendFile)(
        path.join(this.home, "events.jsonl"),
        `${JSON.stringify({
  ...
  async doctor() {
    ...
    for (const project of this.catalog.projects) {
      ...
      for (const root of project.roots ?? []) {
        try {
          await (this.fsApi.access ?? fs.access)(root);
          existingRoots += 1;
        } catch {
          missing.push(root);
        }
