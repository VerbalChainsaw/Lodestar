...
async function readJson(
  ...
  try {
    return JSON.parse(await readFileLimited(file, {
      maximum,
      encoding: "utf8",
...
async function readJsonLines(file, fsApi, optional = false) {
  const text = await readFileLimited(file, {
...
function coverageReport(source, projects, {
  maxAgeDays = 30,
  now = Date.now,
} = {}) {
  if (
    ...
    throw new ContextError("coverage-age-invalid", {
      value: maxAgeDays,
      minimum: 0,
      maximum: 3_650,
    });
  ...
  if (!Number.isFinite(currentTime)) {
    throw new ContextError("coverage-clock-invalid");
  }
  const cutoff = currentTime - maxAgeDays * 24 * 60 * 60 * 1_000;
  const rows = projects.map((project) => {
    ...
    for (const record of records) {
      for (const category of record.covers ?? []) {
    ...
    for (const [category, owners] of categoryRecords) {
      if (owners.length > 0) covered.add(category);
    }
    ...
    for (const [category, owners] of categoryRecords) {
      ...
      const managed = owners.filter(({ ownership }) =>
        ownership === "generated" || ownership === "imported");
      if (managed.length !== owners.length) continue;
      const verificationTimes = managed
        .map(({ verified }) => Date.parse(verified?.at))
        .filter(Number.isFinite);
      if (verificationTimes.length === 0) {
        unverified.push(category);
      } else if (Math.max(...verificationTimes) < cutoff) {
        stale.push(category);
      }
    }
    const complete = missing.length === 0;
    const fresh = stale.length === 0 && unverified.length === 0;
  ...
  return {
    ...
    fresh: reachable.filter(({ fresh }) => fresh).length,
    stale: reachable.filter(({ readiness }) => readiness === "stale").length,
    ready: reachable.filter(({ ready }) => ready).length,
    projects: rows,
  };
}
...
export class ContextStore {
  static async open({
    ...
    const catalog = await readJson(
      ...
      fsApi,
      STORE_LIMITS_V1.maxCatalogBytes,
    );
  ...
  async put(record, {
    ...
    return {
      ...
      ...(update.lock_release ? { lock_release: update.lock_release } : {}),
    };
  }
  ...
  async coverage({
    ...
    return coverageReport(source, projects, { maxAgeDays, now });
  }
  ...
  async doctor() {
    ...
    for (const project of this.catalog.projects) {
      ...
      if (existingRoots === 0 && missing.length > 0) {
        issues.push({
          code: "missing-root",
    ...
    const coverage = coverageReport(source, this.catalog.projects);
    for (const project of coverage.projects) {
      if (!project.blocked && project.missing.length > 0) {
        issues.push({
          code: "project-context-incomplete",
          severity: "warning",
          project: project.id,
          missing: project.missing,
          repair:
            `Run agentctx profile-projects --project ${project.id}, then curate any remaining gaps.`,
        });
      }
      if (!project.blocked && project.stale.length > 0) {
        issues.push({
          code: "project-context-stale",
          severity: "warning",
          project: project.id,
          stale: project.stale,
          repair: `Run agentctx refresh --project ${project.id}.`,
        });
      }
      if (!project.blocked && project.unverified.length > 0) {
        issues.push({
          code: "project-context-unverified",
          severity: "warning",
          project: project.id,
