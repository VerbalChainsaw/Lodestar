...
const MAX_RESULTS_BYTES = 256 * 1024 * 1024;
...
async function readCompleted(resultsFile, plan) {
  try {
    const text = await readFileLimited(resultsFile, {
      maximum: MAX_RESULTS_BYTES,
      encoding: "utf8",
      resource: "category-results-bytes",
    });
    const values = text
      .split(/\r?\n/)
      .filter(Boolean)
...
export async function executeCategoryPlan(plan, {
  ...
  await fs.writeFile(
    ...
    const existing = JSON.parse(await readFileLimited(manifestFile, {
      maximum: MAX_MANIFEST_BYTES,
      encoding: "utf8",
      resource: "category-manifest-bytes",
    }));
    if (
...
export async function loadCategoryConfig(file) {
  try {
    return validateCategoryConfig(JSON.parse(await readFileLimited(file, {
      maximum: MAX_CONFIG_BYTES,
      encoding: "utf8",
      resource: "category-config-bytes",
    })));
  } catch (error) {
