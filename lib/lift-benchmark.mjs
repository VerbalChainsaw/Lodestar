...
async function runLodestarTrial({ home, activeRoot, activeId }) {
  ...
  const startupBytes = Buffer.byteLength(stableJson(startup));
  const resolvedBytes = Buffer.byteLength(stableJson(resolved));
  return {
    ...
    startup_budget_bytes: BUDGETS_V1.start.maxBytes,
  ...
  try {
    return await withLiftBenchmarkFixture(rawOptions, async ({
      ...
      const gates = {
        ...
        startup_payload_within_budget:
          lodestar.startup_bytes <= lodestar.startup_budget_bytes,
      };
      return {
        v: 1,
        ...
        value_observations: {
          inspected_bytes_avoided:
            broad.bytes_inspected - lodestar.bytes_inspected,
          evidence_bytes_added:
            lodestar.evidence_bytes - broad.evidence_bytes,
          median_elapsed_ms_saved: Number(
            (broad.elapsed_ms - lodestar.elapsed_ms).toFixed(3),
          ),
          note:
            "Observed work only; no token, dollar, authoring, or maintenance cost is inferred.",
        },
        gates,
